...
import renderEditQuestion from './rendeEditQuestion';

export default function renderQuestionInAdminPage(question, questionList, adminModal) {
  const answerCorrect =
        ...
        <div class='question-id-${question.id}'>
          ...
          <ul class="answer-list">
            <li class="${answerCorrect.indexOf('a') > -1 ? 'answer-true' : ''}">
              <span>a. <label for="a" id="a_text" >${question.answers.a}</label></span>
            </li>
            <li class="${answerCorrect.indexOf('b') > -1 ? 'answer-true' : ''}">
              <span>b. <label for="b" id="b_text">${question.answers.b}</label></span>
            </li>
            <li class="${answerCorrect.indexOf('c') > -1 ? 'answer-true' : ''}">
              <span>c. <label for="c" id="c_text">${question.answers.c}</label></span>
            </li>
            <li class="${answerCorrect.indexOf('d') > -1 ? 'answer-true' : ''}">
              <span>d. <label for="d" id="d_text">${question.answers.d}</label></span>
            </li>
          </ul>
        </div>

        <div class="quiz-options"> 
          <i class="fa-solid fa-pen question-edit"></i>
  ...
  let deleteBtn = document.querySelector(`.quiz-question-${question.id} .question-delete`);
  let editBtn = document.querySelector(`.quiz-question-${question.id} .question-edit`);
